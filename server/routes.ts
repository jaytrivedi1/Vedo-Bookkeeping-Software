...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Deposit routes
  // Get a specific payment with all related data (for editing)
  apiRouter.get("/payments/:id", async (req: Request, res: Response) => {
    try {
      ...
      // Get the base payment transaction
      ...
      // Get ledger entries to find the deposit account ID and invoice allocations
      const ledgerEntries = await storage.getLedgerEntriesByTransaction(paymentId);
      
      // Find the deposit account (entry with debit > 0)
      const depositEntry = ledgerEntries.find(entry => entry.debit > 0);
      const depositAccountId = depositEntry ? depositEntry.accountId : null;
      
      // Get line items (if any were created)
      let lineItems = await storage.getLineItemsByTransaction(paymentId);
      
      // Check AR entries to see which invoices were paid (if line items don't exist)
      if (lineItems.length === 0) {
        // Look at AR entries (credit > 0 and not related to prepayment)
        const arEntries = ledgerEntries.filter(entry => 
          entry.credit > 0 && entry.accountId === 2 // AR account
        );
        
        // Extract invoice references from descriptions
        for (const entry of arEntries) {
          const match = entry.description.match(/invoice #([^)]+)/);
          if (match && match[1]) {
            // Look up invoice by reference number
            const transactions = await storage.getTransactions();
            const invoice = transactions.find(t => 
              t.reference === match[1] && t.type === 'invoice'
            );
      ...
      const prepaymentEntry = ledgerEntries.find(entry => 
        entry.credit > 0 && entry.accountId === 19 // Business Credit Card / Prepayments
      );
      if (prepaymentEntry) {
        unappliedAmount = prepaymentEntry.credit;
      }
      
      // Construct full payment data
      const fullPayment = {
        ...payment,
        depositAccountId,
        lineItems,
  ...
  apiRouter.patch("/payments/:id", async (req: Request, res: Response) => {
    try {
      ...
      const existingPayment = await storage.getTransaction(paymentId);
      if (!existingPayment || existingPayment.type !== 'payment') {
        return res.status(404).json({ message: "Payment not found" });
      }
      
      // Get existing ledger entries
      const existingLedgerEntries = await storage.getLedgerEntriesByTransaction(paymentId);
      
      // 1. Update the payment transaction itself
      ...
      // 3. Revert the previous invoice payments by reverting balances
      // a. Find A/R entries for paid invoices
      const arEntries = existingLedgerEntries.filter(entry => 
        entry.credit > 0 && entry.accountId === 2
      );
      
      // b. Extract invoice references from descriptions and restore balances
      for (const entry of arEntries) {
        const match = entry.description.match(/invoice #([^)]+)/);
        if (match && match[1]) {
          // Look up invoice by reference number
          ...
          if (invoice) {
            // Add the payment amount back to restore the original balance
            const originalBalance = (invoice.balance || 0) + entry.credit;
            await storage.updateTransaction(invoice.id, {
              balance: originalBalance,
              status: 'pending'
            });
            
            // Delete the old A/R ledger entry
            await storage.deleteLedgerEntry(entry.id);
          }
        }
      }
      
      // 4. Remove any prepayment/unapplied credit entries
      const prepaymentEntry = existingLedgerEntries.find(entry => 
        entry.credit > 0 && entry.accountId === 19
      );
      if (prepaymentEntry) {
        await storage.deleteLedgerEntry(prepaymentEntry.id);
      }
      
      // 5. Apply the updated payments to invoices
      const newLedgerEntries = [];
      if (lineItems.length > 0) {
        for (const item of lineItems) {
          // Skip invalid items
          if (!item.transactionId || item.amount <= 0) continue;
      ...
      // 6. Handle unapplied credit if any
      if (unappliedAmount > 0) {
        const ledgerEntry = await storage.createLedgerEntry({
          transactionId: paymentId,
          accountId: 19, // Business Credit Card (prepayments)
          debit: 0,
          credit: unappliedAmount,
          description: `Unapplied credit for customer #${data.contactId}`,
          date: new Date(data.date),
